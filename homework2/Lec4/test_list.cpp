...
void displayList_v2(ListInterface<std::string> &listobj)
{
   ...
   for(int i=1;i<=listobj.getLength();i++) { // start at 1 and go to the end which is the length of the object
      std::cout << listobj.getEntry(i) << " "; // lists the entries in order in one line
   }
   std::cout << std::endl;
}
...
TEST_CASE("Test copy assignment operator of LinkedList", "[LinkedList]")
{
   ...
   LinkedList<std::string> copyOfList = list; // use copy assignment operator
...
{
   char input;
   std::cout << "Would you like to create an arraylist or linkedlist? ('A' or 'L')";
   std::cin >> input;
   ListInterface<std::string>* listPtr = nullptr; // use a base class

   while(input != 'A' || input != 'L') {
   if (input == 'A') {
      listPtr = new ArrayList<std::string>(); // allocate new memory for an arraylist and have listptr point to it
   }
   else if(input == 'L') {
      listPtr = new LinkedList<std::string>(); // allocate new memory for a linkedlist and have listptr point to it
   }
   else {
      std::cout << "Please input either 'A' or 'L': ";
      std::cin >> input;
   }
   }
